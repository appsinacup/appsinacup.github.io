...
Ok so I started to **stress test** my game server, [Gamend](https://github.com/appsinacup/game_server) (Using k6, a great server stress tool). I wanted to see where the limits are currently, and what is the bottleneck.

For configuration I set it up to execute for 1 minute 5000 **flows**, where each flow creates a new account (**db write**) and then reads 10 times the self user data (**db read**), sleeping 1s in between.
...
![banner](/assets/img/gamend_stress/gamend_arch.png)
...
![first result](/assets/img/gamend_stress/first_results.png)
...
![second result](/assets/img/gamend_stress/second_result.png)
